<?php

declare(strict_types=1);

namespace Dullahan\Asset\Adapter\DependencyInjection;

use Doctrine\Bundle\DoctrineBundle\DependencyInjection\Compiler\DoctrineOrmMappingsPass;
use Symfony\Component\DependencyInjection\Compiler\CompilerPassInterface;
use Symfony\Component\DependencyInjection\ContainerBuilder;

class DoctrineMappingPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container)
    {
        if (!$container->hasExtension('doctrine')) {
            throw new \LogicException('DoctrineBundle is not registered, but it is required by DullahanAssetBundle');
        }

        $namespace = 'Dullahan\\Asset\\Domain\\Entity';
        $path = realpath(dirname(__FILE__) . '/../../Domain/Entity');
        $file = fopen('/var/www/html/Dullahan/var/test4', 'w');
        fwrite($file, json_encode([
            __FILE__,
            realpath(dirname(__FILE__) . '/../../Domain/Entity')
        ], JSON_PRETTY_PRINT));
        fclose($file);

        $mappingDriver = DoctrineOrmMappingsPass::createAttributeMappingDriver(
            [$namespace],
            [$path],
        );

        $mappingDriver->process($container);
    }
}
