...
class FieldReader
{
    ...
    public function __construct(
        ...
        protected EntityCacheServiceInterface $cacheService,
    ...
    public function getFields(
        ...
        $result = $this->getPublicFields($class, $skip);
     ...
     * @return array<string, array<string, mixed>|null>
     */
    protected function getPublicFields(string $class, array $skip = []): array
    {
    ...
    protected function getPublicField(
        ...
        if (is_object($type) && enum_exists($type::class) && property_exists($type, 'value')) {
            $type = $type->value;
        }
    ...
    protected function getNestedPublicFields(
        ...
        $item = $this->cache->getItem($this->cacheService->getEntityFieldCacheKey($type));
        if ($item->isHit()) {
