...
trait EntityUtilSetterTrait
{
     ...
     */
    protected function fillEntity(string $class, object $entity, array $payload, array $definitions): void
    {
    ...
    protected function setOne(
        ...
        if (is_int($item)) {
            $relative = $this->getRepository($class)->find($item);
            if (!$relative) {
     ...
     * @return array<object>
     */
    protected function gatherAllEntities(array $newCollection, string $class, EntityRepository $repo): array
    {
        $ids = [];
        $entities = [];
    ...
    protected function handleNestedEntity(string $class, array $payload): object
    {
        ...
        unset($payload['id']);
        if ($id) {
            return $this->update($class, (int) $id, $payload);
        }

        return $this->create($class, $payload, false);
    }
    ...
    protected function validateRelationOwnership(object $item, string $name): void
    {
        if (
            ...
            && !$item->isOwner($this->user ?? $this->userService->getLoggedInUser())
        ) {
            throw new \Exception(
